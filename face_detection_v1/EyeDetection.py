...
IrisRightCorner_Index = [173,263,226] # [left, right] 226(left eve left corner)
...
def DrawPoint():
    ...
    for facePoint_Index, facePoint_location in enumerate(face_landmarks.landmark):
        ...
        if facePoint_Index in PointOfHeadTracking_Index: #in EyeCenter_Index or facePoint_Index in IrisRightCorner_Index:  # # Nose tip and start points Pupil Center Points
            ...
            posx = DetectEyePositionOnScreen()[0] 
            
            if MovedMouse == False:
                #pyautogui.moveTo(posx, height/2)
                MoveMouse() #use for head tracking
...
def DetectEyePositionOnScreen():
    ...
    #posX = reMap(GetDistance(IrisRightCorner_Index[2],EyeCenter_Index[0])[0],30,20,width,0) #GetDistance(IrisRightCorner_Index[2],EyeCenter_Index[0])[0] 
